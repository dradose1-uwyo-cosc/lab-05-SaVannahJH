...
# Lab 03 
# Lab Section: 14
# Sources, people worked with, help given to: TA, Assistant TA, COSC1010_lec7-IfStatements.pptx.pdf
# your
# comments
# here
...
maximum_value = max_temps[0]
minimum_value = min_temps[0]
for index in range (len(max_temps)):
    if max_temps[index] > maximum_value:
        maximum_value = max_temps[index]
print(f"Max temp = {maximum_value}")

for index in range (len(min_temps)):
    if min_temps[index]< minimum_value:
        minimum_value = min_temps[index]
print(f"Min temp = {minimum_value}")
...
print(f"Zero occurred {zero_num} time(s)")
